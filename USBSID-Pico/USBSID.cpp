...
unsigned char USBSID_Class::USBSID_SingleReadConfig(unsigned char *buff, int len)
{
  if (!us_PortIsOpen) return 0;
  int actual_length;
  rc = libusb_bulk_transfer(devh, EP_IN_ADDR, buff, len, &actual_length, 3000);
  if (rc == LIBUSB_ERROR_TIMEOUT) {
    USBERR(stderr, "[USBSID] Timeout error while reading (%d)\n", actual_length);
    return 0;
  } else if (rc < 0) {
...
void USBSID_Class::LIBUSB_CloseDevice(void)
{
  ...
  if (devh) {
      int start_if, end_if;
  #ifdef USE_RAW_USBIF
      /* e.g. macOS needs RAW interface 4, all others can stay with 0, 1 */
      start_if = 4;
      end_if = 5;
...
int USBSID_Class::LIBUSB_DetachKernelDriver(void)
{
   ...
   */
    int start_if, end_if;
    /* macOS needs RAW interface 4, all others can stay with 0, 1 */
#ifdef __APPLE__
    start_if = 4;
    end_if = 5;
#else
...
int USBSID_Class::LIBUSB_ConfigureDevice(void)
{
  USBDBG(stdout, "[USBSID] Configure device\r\n");
    ...
    rc = libusb_control_transfer(devh,
        0x21,       /* Host->Interface, Class, Interface */
        0x22,       /* SET_CONTROL_LINE_STATE */
        0x01,       /* value */
        4,          /* index = Interface 4, not 0 */
        NULL,
        0,
        1000);
#else
...
void USBSID_Class::LIBUSB_StopTransfers(void)
{
  ...
  if (transfer_in != NULL) {
    ...
    while (transfer_out != NULL || transfer_in != NULL) {
...
{
    if (transfer->status != LIBUSB_TRANSFER_COMPLETED) {
         int _rc = transfer->status;

         if (_rc != LIBUSB_TRANSFER_CANCELLED) {
             USBERR(stderr,
