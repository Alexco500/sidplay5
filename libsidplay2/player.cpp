...
/***************************************************************************
 *  $Log: player.cpp,v $
 *  Revision 1.3.2.3  2004/08/02 21:18:52  sid
 *  fixed hardsid support (actually tested with a HardSID now), added proper
 *  GUI-disabling for HardSID (and also Catweasel).
 *
 *  Revision 1.3.2.2  2004/07/26 18:11:10  sid
 *  synced with libsidplay2 from cvs and resid 0.16
 *
...
:c64env  (&m_scheduler),
 ...
 m_sampleCount       (0),
 m_RegisterFrameChangedCallback(NULL),
 m_RegisterFrameChangedCallbackInstance(NULL)
{
...
void Player::writeMemByte_playsid (uint_least16_t addr, uint8_t data)
{
    ...
    else // Mirrored SID.
    {
		...
		// AV
		if ( ( tempAddr & 0xff ) <= 0x18 )
		{
			m_CurrentRegisterFrame.mRegisters[tempAddr & 0xff] = data;
			m_CurrentRegisterFrame.mTimeStamp = m_scheduler.getTime(EVENT_CLOCK_PHI2);
			if (m_RegisterFrameChangedCallback)
				m_RegisterFrameChangedCallback(m_RegisterFrameChangedCallbackInstance, m_CurrentRegisterFrame);
		}
    }
}
