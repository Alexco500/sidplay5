...
/* $Id: util.c,v 1.117.2.1 2005/11/20 14:08:25 bouvigne Exp $ */
       ...
       int ch) 
{
  ...
  if ( gfc->fill_buffer_resample_init == 0 ) {
    gfc->inbuf_old[0] = (sample_t*) calloc(BLACKSIZE,sizeof(gfc->inbuf_old[0][0]));
    gfc->inbuf_old[1] = (sample_t*) calloc(BLACKSIZE,sizeof(gfc->inbuf_old[0][0]));
    for (i=0; i<=2*bpc; ++i)
      gfc->blackfilt[i] = (sample_t*) calloc(BLACKSIZE,sizeof(gfc->blackfilt[0][0]));
