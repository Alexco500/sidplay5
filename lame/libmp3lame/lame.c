...
lame_init_params(lame_global_flags * const gfp)
{
    ...
    if (NULL == gfc->PSY)
        gfc->PSY = (PSY_t*) calloc(1, sizeof(PSY_t));
    if (NULL == gfc->PSY)
        return -2;
        
    if (NULL == gfc->rgdata)
        gfc->rgdata = (replaygain_t*) calloc(1, sizeof(replaygain_t));
    if (NULL == gfc->rgdata)
        return -2;
   ...
   *******************************************************/
	...
	if (gfp->exp_nspsytune2.pointer[0])
	    gfc->nsPsy.pass1fp = (FILE*) gfp->exp_nspsytune2.pointer[0];
	else
...
update_inbuffer_size(lame_internal_flags * gfc, const int nsamples)
{
    if (gfc->in_buffer_0 == 0 || gfc->in_buffer_nsamples < nsamples) {
        ...
        gfc->in_buffer_0 = (sample_t*) calloc(sizeof(sample_t), nsamples);
        gfc->in_buffer_1 = (sample_t*) calloc(sizeof(sample_t), nsamples);
...
lame_init_old(lame_global_flags * gfp)
{
    ...
    if (NULL ==
        (gfc = gfp->internal_flags =
         (lame_internal_flags*) calloc(1, sizeof(lame_internal_flags)))) return -1;
